...
headers = ['High/Low', 'Letter', 'Password']

df = pd.DataFrame(list(map(str.split, raw_input.splitlines())), columns = headers)

# Split High/Low into two Series 
new = df["High/Low"].str.split("-", n = 1, expand = True)
df["Low"] = new[0].astype(int)
df["High"] = new[1].astype(int)
df.drop(columns="High/Low", inplace=True)

# Remove : from Letter Series 
df['Letter'] = df['Letter'].str.replace(':', '')    

# Create Letter Count of letter in Password 
df['Letter Count'] = df.apply(lambda row : row['Password'].count(row['Letter']), axis = 1)

# 636 
df['Letter Count'].between(df['Low'], df['High'], inclusive=True).sum()
